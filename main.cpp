...
#if defined(NDEBUG)
# error "Universal cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x18f2fa7772d0b4fab416282652ebb135891ae76fbb9f295e1bce285b09b8c273");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Universal: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Universal Signed Message:\n";
...
{
    // Universal: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
     ...
     if(nHeight ==1) {
       nSubsidy = 9890100000 * COIN ;
     }
   if(nHeight >=2 && nHeight<=9881280) 
    {
        nSubsidy = 0.1 * COIN;
    }
...
static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Universal: 3.5 days
static const int64 nTargetSpacing = 60; // Universal: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Universal: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Universal: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x74152d56c08c7f98c9ab9e948031d0f3b70f8b79355e94546055b095453c7bd7");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Universal coin make by 2018 in korea";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1544016236;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2092328356;

        if (fTestNet)
        {
            block.nTime    = 1544016221;
            block.nNonce   = 395909451;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x7a2d715e51d9de7cdbe59604a9d48aa5c1a1aaa78d572eb3a2569d4bf89119e9"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xb4, 0xcd, 0xdb }; // Universal: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("UniversalMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("UniversalMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static UniversalMiner(CWallet *pwallet)
{
    printf("UniversalMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("universal-miner");
    ...
    try { loop {
        ...
        printf("Running UniversalMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("UniversalMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&UniversalMiner, pwallet));
}
